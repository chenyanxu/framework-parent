...
public class ImageScale {
    public static int thumMaxWidth = 200;
    ...
    public BufferedImage imageZoomOut(BufferedImage srcBufferImage) {
        width = srcBufferImage.getWidth();
        height = srcBufferImage.getHeight();

        if (width > thumMaxWidth) {
            width = thumMaxWidth;
        }
        return imageZoomOut(srcBufferImage, width, height, true);
    }
